...
export default async function action(query?: EventsQuery): Promise<EventsProcessed> {
  ...
  const totalEvents = events.reduce((acc, event) => acc + event.eventsOnBlock.length, 0);
  ...
  try {
    for (let event of events) {
      ...
      const wasRegistered = !!eventsOnBlock.find(e => e.returnValues.network === network.networkAddress);
